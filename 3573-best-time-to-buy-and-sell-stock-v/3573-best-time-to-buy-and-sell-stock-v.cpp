...
public:
    ...
    long long maximumProfit(vector<int>& prices, int k) {
        int n = prices.size();
        vector<vector<vector<long long>>> dp(n+1, vector<vector<long long>>(3, vector<long long>(k+1, LLONG_MIN/2)));

        for(int tx = 0; tx <= k; tx++){
            dp[n][0][tx] = 0;
            dp[n][1][tx] = dp[n][2][tx] = LLONG_MIN / 2;
        }

        for(int i = n-1; i >= 0; i--){
            for(int transType = 0; transType <= 2; transType++){
                for(int tx = 0; tx <= k; tx++){
                    // option-1: skip
                    long long ans = dp[i+1][transType][tx];
                    // option-2: Buy
                    if(transType == 0){   
                        // no active tranasction => Begin Normal transaction
                        ans = max(ans, -prices[i] + dp[i+1][1][tx]);
                        // no active tranasction => Begin Short selling transaction
                        ans = max(ans, prices[i] + dp[i+1][2][tx]);
                    }
                    else if(transType == 2 && tx > 0){   // active transaction => Short selling transaction => buy and complete the transaction
                        ans = max(ans, -prices[i] + dp[i+1][0][tx-1]);
                    }
                    // option-3: Sell
                    else if(transType == 1 && tx > 0){   // active transaction => Normal transaction => Sell and finish the transaction
                        ans = max(ans, prices[i] + dp[i+1][0][tx-1]);
                    }

                    dp[i][transType][tx] = ans;
                }
            }
        }
        return dp[0][0][k];  // index, transType{0: no active, 1: normal tx, 2: short selling txn}
    }
};
